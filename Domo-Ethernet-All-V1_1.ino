...
//Esta utilidad es especifica en base a la placa de red que utilicemos. Me sirve para controlar el tema de los timeout
...
HumTempDHT humTemps[humTempSize]= {HumTempDHT(8, 11)};
#define tempSize 2
...
DigitalControl digitals[digitalSize]= {DigitalControl(9, 2)};
...
void controllerInfo(HttpRequest &httpRequest){
  ...
  if(httpRequest.getMethod() == "PUT"){
    String value= parseProperty(httpRequest.getPunteroBody(), "tiS", 6);
    if(isNotNull(value)){ timeSensors= value.toInt(); }  
    value= parseProperty(httpRequest.getPunteroBody(), "tiC", 6);
    if(isNotNull(value)){ timeControls= value.toInt(); }
    
    updateEthernet(httpRequest); 
